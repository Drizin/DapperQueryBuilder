...
If your provider doesn't accept at-parameters you can modify `DapperQueryBuilderOptions.AutoGeneratedParameterPrefix` and `DapperQueryBuilderOptions.AutoGeneratedParameterObjectPrefix`:

```cs
// Default value is "@p", which mean the underlying query will use @p0, @p1, etc..
// Some database vendors (like Oracle) expect ":" parameters instead of "@" parameters
DapperQueryBuilderOptions.AutoGeneratedParameterPrefix = ":p";
...
If for any reason you don't want parameters to be named `p0`, `p1`, etc, you can change the auto-naming by setting `AutoGeneratedParameterPrefix` to whatever you like and setting `AutoGeneratedParameterObjectPrefix` accordingly:

```cs
DapperQueryBuilderOptions.AutoGeneratedParameterPrefix = "@PARAM_";
DapperQueryBuilderOptions.AutoGeneratedParameterObjectPrefix = "PARAM_";
